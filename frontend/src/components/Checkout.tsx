...
const { items, totalPrice, clear } = useCart()
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const id = 'AYSHO-' + resData.data.documentId.slice(-6).toUpperCase()
      setOrderId(id)
      setSubmitted(true)
      clear()
    } catch (err) {
  ...
  if (submitted) {
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-brand-900/50 p-4 backdrop-blur-sm">
        <div className="animate-fade-up w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto bg-brand-900/50 p-4 backdrop-blur-sm sm:items-center">
      <div className="animate-fade-up my-4 w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="max-h-[75vh] overflow-y-auto">
          <div className="grid gap-6 px-6 py-6 md:grid-cols-5">
            ...
            <div className="space-y-4 md:col-span-3">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Ces informations sont envoyées à l'administrateur pour traiter votre commande.
                </p>
              </div>
          ...
          <div className="border-t border-sand-200 bg-sand-50 px-6 py-4">
            ...
            <p className="mt-2 text-center text-xs text-slate-500">
              En validant, vous acceptez d'être contacté par AYSHO pour la livraison.
            </p>
          </div>
        </form>
      </div>
